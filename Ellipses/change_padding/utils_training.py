...
def reflexive_padding_2d(W: tf.Tensor, pad_1: int, pad_2: int, mode: str):
    ...
    Parameters:
        ...
        mode (str): The type of padding to apply.

    Returns:
        tf.Tensor: The padded tensor.
    """
    return tf.pad(
        W, [[0, 0], [pad_1, pad_1], [pad_2, pad_2], [0, 0]], mode=mode
    )
...
class FNO2d(tf.keras.Model):
    ...
    def __init__(
        self, modes: int, width: int, pad_prop=0.05, pad_mode="REFLECT"
    ):
        """
    ...
    @tf.function
    def call(self, A):
        if self.pad_prop != 0.0:
            pad_1 = int(A.shape[1] * self.pad_prop)
            pad_2 = int(A.shape[2] * self.pad_prop)

            A = reflexive_padding_2d(A, pad_1, pad_2, self.pad_mode)
