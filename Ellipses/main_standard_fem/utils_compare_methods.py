...
class StandardFEMSolver:
    ...
    def solve_one(self, i, size, nb_vert=64, reference_fem=False):
        ...
        print(
            f"(mu0, mu1, sigma_x, sigma_y, amplitude, x_0, y_0, lx, ly, theta, alpha, beta) = {self.params[i]}"
        )

        # construction of the mesh
        if reference_fem:
            XX, YY = np.meshgrid(np.linspace(0.0, 1.0, 512), np.linspace(0.0, 1.0, 512))
            XX = np.reshape(XX, [-1])
            YY = np.reshape(YY, [-1])
            XXYY = np.stack([XX, YY])
...
class PhiFemSolver_error:
    ...
    def solve_one(self, i):
        ...
        print(
            f"(mu0, mu1, sigma, x_0, y_0, amplitude, lx, ly, theta, alpha, beta) = {self.params[i]}"
        )
