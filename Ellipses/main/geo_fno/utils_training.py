...
class DataLoader:
    def __init__(self, small_data=False, dtype=torch.float32):
        ...
        input_f = np.load("../../data/F_geo_fno.npy")[:, :, None]
        input_g = np.load("../../data/G_geo_fno.npy")[:, :, None]
...
class Agent:
    def __init__(self, data):
        ...
        self.optimizer_fno = Adam(
            self.model.parameters(), lr=self.initial_lr, weight_decay=1e-4
        )
        ...
        self.optimizer_iphi = Adam(
            self.model_iphi.parameters(), lr=self.initial_lr_iphi, weight_decay=1e-4
        )
        ...
        self.loss_function = LpLoss(size_average=True)
    ...
    def eval_losses(self):
        ...
        loss_T = loss_T / nb_test_batches
        loss_V = loss_V / nb_test_batches
        self.losses_dict.append(
            {
                "epoch": self.nb_train_epochs,
                "loss_T": f"{(loss_T.item()):.3e}",
                "loss_V": f"{(loss_V.item()):.3e}",
            }
        )
