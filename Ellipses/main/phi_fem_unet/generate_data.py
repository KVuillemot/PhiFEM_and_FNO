...
def generate_parameters(nb_data=1800, nb_vert=64, save=True):
    F, phi, G, params = create_parameters(nb_data=nb_data, nb_vert=nb_vert)
    if save:
        if not (os.path.exists(f"../../data")):
            os.makedirs(f"../../data")
        if not (os.path.exists(f"../../data_test")):
            os.makedirs(f"../../data_test")
        np.save(f"../../data/G.npy", G[:1800])
        np.save(f"../../data/F.npy", F[:1800])
        np.save(f"../../data/Phi.npy", phi[:1800])
        np.save(f"../../data/params.npy", params[:1800])

        np.save(f"../../data_test/G.npy", G[1800:])
        np.save(f"../../data_test/F.npy", F[1800:])
        np.save(f"../../data_test/Phi.npy", phi[1800:])
        np.save(f"../../data_test/params.npy", params[1800:])
...
if __name__ == "__main__":
    params = generate_parameters(2100)
    go(params)
