...
class PhiExpr(MyUserExpression):
    ...
    def eval(self, value, x):
        value[0] = call_phi(
            dol, x, self.x_0, self.y_0, self.lx, self.ly, self.theta
        )
...
class PhiFemSolver:
    ...
    def solve_one(self, i):
        mu0, mu1, sigma, x_0, y_0, lx, ly, theta = self.params[i]
...
def go():
    ...
    print(np.shape(phi * W))
    print("U", np.min(phi * W), np.max(phi * W))
    ...
    for i in range(nb):
        mu0, mu1, sigma, x_0, y_0, lx, ly, theta = params[i]
        domain = omega_mask(nb_vert, x_0, y_0, lx, ly, theta)
        axs[i, 0].imshow(F[i, :, :] * domain, origin="lower")
        axs[i, 1].imshow(phi[i, :, :] * domain, origin="lower")
        axs[i, 2].imshow(W[i, :, :] * domain, origin="lower")
        axs[i, 3].imshow(phi[i, :, :] * W[i, :, :] * domain, origin="lower")
        axs[i, 0].set_title("F")
