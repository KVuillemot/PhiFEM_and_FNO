...
class DataLoader:
    ...
    def __init__(self, small_data=False, dtype=torch.float32):
        ...
        domains_nan = domain.copy().astype(float)  # .flatten().astype(float)
        ...
        for epoch in range(1, nb_epochs + 1):
            ...
            if epoch % 1 == 0:
                ...
                if mean_loss_validation <= best_mean_loss_validation:
                    ...
                    file.write(
                        f"(level, initial_lr, n_modes, width, batch_size, l2_lambda, pad_prop, pad_mode) = "
                        + f"{(self.level, self.initial_lr, self.n_modes, self.width, self.batch_size, self.l2_lambda, self.pad_prop, self.pad_mode)} \n"
                    )
