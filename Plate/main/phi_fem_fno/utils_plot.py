...
def plot_meshes_pyvista(meshes, labels=[]):
    ...
    subplotter.subplot(0, 0)
    colors = [[0.6, 0.2, 0.1], [0.1, 0.6, 0.6], [0.1, 0.6, 0.1]]
    for i in range(len(meshes)):
        if len(labels) == 0:
            label = ""
        else:
...
def plot_scalar_functions_list(U, labels=[], screenshot=False, height=400):

    nb_functions = len(U)
    width = height * nb_functions
    subplotter = pyvista.Plotter(
...
def plot_2D_vector_function_list_v2(
    U, labels=[], mode="displaced", screenshot=False, height=400, shape=None
):
    ...
    if type(mode) == list:
        ...
        for i in range(len(mode)):
            if type(mode[i]) == str:
                ...
                assert type(mode[i]) == int or type(mode[i]) == float
                factor.append(mode[i])
    else:
        factor = []
        for i in range(nb_functions):
            if type(mode) == str:
                ...
                assert type(mode) == int or type(mode) == float
                factor.append(mode)
    ...
    for index, u in enumerate(U):
        ...
        grid.point_data["Displacement"] = u_3D
        grid.set_active_vectors("Displacement")
        warped = grid.warp_by_vector("Displacement", factor=factor[index])

        if index > 0:
            error = np.zeros((x.shape[0], 3))
            error[:, :2] = np.absolute(U[0].x.array - u.x.array).reshape(-1, 2)
        ...
        if index == 0:
            ...
            subplotter.add_mesh(
                warped, scalars=error, show_edges=False, show_scalar_bar=False
            )
        subplotter.add_scalar_bar(
            title=empty_str,
            height=0.1,
