...
class DataLoader:
    ...
    def __init__(self, small_data=False, dtype=torch.float32):
        ...
        G = torch.tensor(np.load(f"../../data/G.npy"), dtype=dtype)
        Y = torch.tensor(np.load(f"../../data/U.npy"), dtype=dtype)
        ...
        Phi_val = torch.tensor(np.load(f"../../data_val/Phi.npy"), dtype=dtype)
        G_val = torch.tensor(
            np.load(f"../../data_val/G.npy"),
            dtype=dtype,
        )
        Y_val = torch.tensor(np.load(f"../../data_val/U.npy"), dtype=dtype)
        X_val = torch.stack([Phi_val, G_val[:, 1, :, :]], dim=1)
        print(f"{G_val.shape=}")
        print(f"{Y_val.shape=}")
        print(f"{Phi_val.shape=}")
        print(f"{X_val.shape=}")
        print(f"{Y_val.shape=}")
        self.X_val = X_val
        self.Y_val = Y_val
        ...
        loss = Loss()
        domain_tmp = self.X_train[:, 0, :, :] <= 3e-16
        ...
        self.Y_train_nan = domains_nan * self.Y_train
        self.y_normalizer = UnitGaussianNormalizer(self.Y_train_nan)
        ...
        self.nb_train, self.nb_val = self.X_train.shape[0], self.X_val[0]
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft[:, :, : self.modes_x, : self.modes_y] = factor1
        out_ft[:, :, -self.modes_x :, : self.modes_y] = factor2
...
class FNO2d(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.nb_layers):
            self.layers.append(
                FNO2dLayer(
                    SpectralConv2d(self.width, self.width, self.modes_x, self.modes_y),
                    nn.Conv2d(self.width, self.width, 1),
                )
            )

        self.fc1 = nn.Conv2d(self.width, 128, 1)
        self.fc2 = nn.Conv2d(128, 2, 1)
    ...
    def get_grid(self, shape, device):
        ...
        gridy = torch.tensor(np.linspace(0.25, 0.75, size_y), dtype=torch.float)
    ...
    def forward(self, x):
        ...
        """
        self.padding_x = int(x.shape[2] * self.pad_prop)
        self.padding_y = int(x.shape[3] * self.pad_prop)

        grid = self.get_grid(x.shape, x.device)
        x = torch.cat((x, grid), dim=1)
        x = self.fc0(x)
        if self.padding_x != 0 and self.padding_y != 0:
            x = torch.nn.functional.pad(
                x,
                [self.padding_y, self.padding_y, self.padding_x, self.padding_x],
                mode=self.pad_mode,
            )
        for i in range(self.nb_layers):
            x = self.layers[i](x)
        ...
        if self.padding_x != 0 and self.padding_y != 0:
            x = x[
                ..., self.padding_x : -self.padding_x, self.padding_y : -self.padding_y
            ]
        x = self.fc1(x)
        x = self.activation(x)
        x = self.fc2(x)
...
class Agent:
    ...
    def __init__(
        ...
        self.model = FNO2d(
            in_channels,
            modes_x=self.n_modes,
            modes_y=self.n_modes,
            width=self.width,
    ...
    def eval_losses(self):
        ...
        with torch.no_grad():
            ...
            loss_T, loss_0_T, loss_1_T, loss_2_T = self.loss_function(
                ...
                relative=self.relative,
                squared=False,
                plot=self.nb_train_epochs % 50 == 0,
            )
            ...
            self.losses_dict = [
                {
                    ...
                    "rel-L2/Loo_T": f"{(loss_1_T.item() ):.2e}",
                    "loss_V": f"{(loss_v.item() ):.2e}",
                    "rel-L2_V": f"{(loss_0_v.item() ):.2e}",
                    "rel-L2/Loo_V": f"{(loss_1_v.item() ):.2e}",
                }
            ]
        else:
            self.losses_dict = [
    ...
    def train(self, nb_epochs=2000, models_repo="./models", save_models=True):
        ...
        for epoch in range(1, nb_epochs + 1):
            ...
            if self.scheduler.lr_has_changed:
                if self.scheduler.patience <= 90:
                    self.scheduler.patience = int(self.scheduler.patience * 1.4)
                print(
