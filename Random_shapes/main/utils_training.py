...
def test_domain_and_border():
    ...
    """
    phi = torch.tensor(np.load("../data_reduced/Phi.npy"), dtype=torch.float32)[
        None, 20, :, :
    ]
    tmp_loss = Loss()
...
def test_data():
    ...
    phi = torch.tensor(np.load("../data_reduced/Phi.npy"), dtype=torch.float32)[10]
    F = torch.tensor(np.load("../data_reduced/F.npy"), dtype=torch.float32)[10]
    W = torch.tensor(np.load("../data_reduced/W.npy"), dtype=torch.float32)[10]
    G = torch.tensor(np.load("../data_reduced/G.npy"), dtype=torch.float32)[10]
...
class DataLoader:
    ...
    def __init__(self, small_data=False, dtype=torch.float32):
        ...
        F = torch.tensor(np.load(f"../data_reduced/F.npy"), dtype=dtype)
        nb_vert = F.shape[1]
        self.nb_vert = nb_vert
        Phi = torch.tensor(np.load(f"../data_reduced/Phi.npy"), dtype=dtype)
        G = torch.tensor(
            np.load(f"../data_reduced/G.npy"),
            dtype=dtype,
        )
        Y = torch.tensor(np.load(f"../data_reduced/W.npy"), dtype=dtype)[:, None, :, :]
