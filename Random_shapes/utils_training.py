...
def test_domain_and_border():
    phi = tf.constant(
        np.load("./data_2811_3_modes/Phi_64.npy"), dtype=tf.float32
    )[3]
...
def test_data():
    ...
    F = tf.constant(np.load("./data_2811_3_modes/F.npy"), dtype=tf.float32)[10]
    W = tf.constant(np.load("./data_2811_3_modes/W.npy"), dtype=tf.float32)[10]

    domain, boundary = domain_and_border(np.shape(phi)[0], phi)
    plt.figure(figsize=(9, 6))
    plt.subplot(2, 3, 1)
    plt.imshow(phi <= 0.0, cmap=my_cmap)
    plt.grid(False)
    plt.subplot(2, 3, 2)
    plt.imshow(boundary, cmap=my_cmap)
    plt.grid(False)
    ...
    plt.title("W")
    plt.subplot(2, 3, 5)
    plt.imshow(W * domain * phi, cmap=my_cmap)
    plt.grid(False)
    plt.colorbar(shrink=0.6)
    plt.title("U")
    plt.tight_layout()
...
class DataLoader:
    def __init__(self, small_data=False, dtype=tf.float32):
        ...
        self.level_set_type = ""
        F = tf.constant(
            np.load(f"./data_{self.nb_data}_3_modes/F.npy"),
            dtype=dtype,
        )
        ...
        F = tf.constant(
            np.load(f"./data_{self.nb_data}_3_modes/F.npy") / self.max_norm_F,
            dtype=dtype,
        )

        Phi = tf.constant(
            np.load(f"./data_{self.nb_data}_3_modes/Phi_64.npy"),
            dtype=dtype,
        )  # (b,nb_vert,nb_vert)
        Y = tf.constant(
            np.load(f"./data_{self.nb_data}_3_modes/W.npy"),
            dtype=dtype,
        )[:, :, :, None]
...
class Agent:
    ...
    def show_results(self):
        ...
        U_fe = Y_val[:, :, :, 0] * X_val[:, :, :, 1]
        U_pi = Y_pred[:, :, :, 0] * X_val[:, :, :, 1]
